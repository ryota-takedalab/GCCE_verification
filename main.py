...
def main():
    parser = argparse.ArgumentParser(
        prog="main.py",
        usage="python main.py training.csv [--pos_fps FPS(int)] [--rot_fps FPS(int)] [--no_pos] [--no_rot] [-t TRIAL(int)] [--lower] [--plot]",
        add_help=True,
    )
    ...
    parser.add_argument(
        "-t", "--trial", help="trial(int): number of trials of random validation.", type=int, default=1000
    )
    ...
    acc_list = []
    f1_list = []
    all_coef = np.zeros(shape=(n_key, n_frame, trial))
    for i in tqdm(range(trial)):
        # split into training and validation data (33% validation set)
        ...
        tqdm.write(f"--[trials {i + 1}]--------------------------------------")
        tqdm.write(f"Acc : {format(acc, '.2f')}%")
        f1 = f1_score(y_test, y_predict) * 100
        tqdm.write(f"F-Measure : {format(f1, '.2f')}%")
        ...
        if args.plot:
            ...
            for frame in range(n_frame):
                for k in range(n_key):
                    all_coef[k][frame][i] = regression_coef[x_n] + regression_coef[y_n] + regression_coef[z_n]
                    x_n += 3
                    y_n += 3
                    z_n += 3

    print("--[Random Validation]------------------------------")
    print(f"Avarage Acc: {format(np.sum(acc_list) / trial, '.2f')} ±{format(np.std(acc_list), '.2f')}%")
    print(f"Avarage F-Measure: {format(np.sum(f1_list) / trial, '.2f')} ±{format(np.std(f1_list), '.2f')}%")
